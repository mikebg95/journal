...
| **list of tags** | can be empty (length 0); max length 10 | Edit DTO: `@Size(max = 10)`. Not on create DTO. | Remove null values; remove empty strings; check max length (10) → remove excess | — |
| **list of todos** | can be empty (length 0); max length 20 | Edit DTO: `@Size(max = 20)`. Not on create DTO. | Remove null values; remove empty strings; check max length (20) → remove excess | — |
