...
| **mood** | not blank/empty → becomes null; null allowed; enum check (one of six values or null) | — | Trim (outer); uppercase; if empty/blank → null; check enum allowed (else null) | `VARCHAR(20)`, `CHECK (mood IN ('HAPPY','CALM',...))`, nullable |
