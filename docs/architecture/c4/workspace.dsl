workspace "Notes" "A personal administrative AI-generated tool to organize journal entries." {
    model {
        user = person "User" "A person who inserts, edits, deletes and browses their personal journal entries."

        notes = softwareSystem "Notes" "Allows users to create, browse, and manage their journal entries. Also generates information about entries (summary, tags, mood) based on content." {
            ...
            backend = container "Backend API" "Handles notes CRUD operations and generates relevant information about journal entries." "Java, Spring Boot, Spring AI" {
                web = component "Web Adapter" "Is the entry point to the backend API" "Spring MVC"
                domain = component "Core Domain" "Business logic for notes and tags; defines the ports the adapters implement" "Plain Java"
    ...
    views {
        systemContext notes "SystemContext" {
            include *
            autolayout lr
        }

        container notes "Containers" {
            include *
            autolayout lr
        }
